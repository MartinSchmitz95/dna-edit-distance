...
def global_clusters(args, small_emb_file, n_processors):
    ...
    clusters = KMeans(n_clusters=n_processors, max_iter=5).fit(embs).labels_
    return clusters
...
def local_clusters(args, cluster_indices, large_emb_file, n_processors):
    ...
    for i in range (n_processors):
        ...
        clustering = AgglomerativeClustering(n_clusters=None, distance_threshold=2).fit(embs_in_cluster).labels_
...
def simple_clusters(emb_file):
    with open(os.path.join(args.data_path,f'{emb_file}.p'), 'rb') as handle:
        embs = pickle.load(handle)
    #clusters = KMeans(n_clusters=22).fit(embs).labels_
    clusters = AgglomerativeClustering(n_clusters=None, distance_threshold=2).fit(embs).labels_
    #clusters = AgglomerativeClustering(n_clusters=22).fit(embs).labels_
...
def get_args():
    ...
    #parser.add_argument("--data_samples", type=str, default='clustered-nanopore-reads-dataset/test_split_20/test_Clusters.txt', help="dataset")
    ...
    parser.add_argument("--model_small", type=str, default='models/emb_dim_8.pt', help="model")  # 'shuffled_dna_strings'
    parser.add_argument("--model_large", type=str, default='models/emb_dim_64.pt', help="model")  # 'shuffled_dna_strings'
...
if __name__ == "__main__":
    ...
    create_files(args, "synth01_real_clusters", "synth01_8_embs", "synth01_64_embs")

    #parallel_clustering(args, n_processors, "synth01_real_clusters", "synth01_8_embs", "synth01_64_embs")
    #simple_clustering(args, "synth01_real_clusters", "synth01_64_embs")
    simple_clustering(args, "testset_real_clusters", "testset_64_embs")
